...
import { vikePreset, nextPreset, builtinFrameworkPresets, FrameworkPresetRegistry, builtinFrameworkPresetRegistry } from './library.js'

describe('built-in presets', () => {
  it('ship Vike (flagship) and Next, each a pure detector', () => {
    assert.deepEqual(builtinFrameworkPresets().map(p => p.name), ['vike', 'next'])
...
describe('FrameworkPresetRegistry', () => {
  it('selects the detected preset', () => {
    const { preset, detection } = builtinFrameworkPresetRegistry().select({ dependencies: ['next'] })
  ...
  it('falls back to the flagship (first-registered) preset when nothing matches', () => {
    const { preset, detection } = builtinFrameworkPresetRegistry().select({ dependencies: ['express'] })
  ...
  it('honors an explicit fallback', () => {
    const { preset } = builtinFrameworkPresetRegistry().select({ files: [] }, nextPreset)
    assert.equal(preset.name, 'next')
  })

  it('get / all / add', () => {
    const reg = new FrameworkPresetRegistry([vikePreset])

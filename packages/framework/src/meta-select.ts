...
export function metaSelectPrompt(intent: string, catalog: readonly PresetCatalogEntry[], workspace: string): string {
  ...
  return [
    ...
    'Pick "none" when the task has no concrete goal — a vague or open-ended request such as "clean this up",',
    '"make it better", or "take a look". A domain review loop only helps once the goal is clear; do not guess',
    'a preset from the workspace alone when the task itself is unspecified.',
